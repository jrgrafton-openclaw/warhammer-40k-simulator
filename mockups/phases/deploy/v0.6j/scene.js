...
(function addSvgBoardSurface() {
  ...
  // ── Edge vignette — wider + matched to darkened fog-base ──
  var VIGNETTE_DEPTH = 180; // wider fade for softer transition
  var vigColor = '#0e1218'; // matches darkened fog-base exactly

  // Helper: create linear gradient for each edge direction
  var gradDefs = [
  ...
  gradDefs.forEach(function(g) {
    ...
    // Two-stop gradient: opaque at edge, transparent inward
    lg.innerHTML = '<stop offset="0%" stop-color="' + vigColor + '" stop-opacity="0.95"/>' +
      '<stop offset="40%" stop-color="' + vigColor + '" stop-opacity="0.5"/>' +
      '<stop offset="100%" stop-color="' + vigColor + '" stop-opacity="0"/>';
    defs.appendChild(lg);
  });

  // Vignette group — appended AFTER terrain so it overlays everything
