/**
 * scene.js — Army data + initialisation wiring for deployment v0.6c.
 * ES module entry point. Imperium units start in the SVG Staging zone.
 * Ork units are pre-deployed on the board.
 * Includes PixiJS canvas layer for reference-grade visuals.
 */
...
import { initDeployment } from './deployment.js?v=20260316-v06c';
import { initPixiLayer } from './pixi-layer.js';
...
// ── Initialise PixiJS canvas layer (behind SVG) ─────────
initPixiLayer().then(function(app) {
  if (app) console.log('[v0.6c] PixiJS layer initialised');
}).catch(function(err) {
  console.warn('[v0.6c] PixiJS init failed:', err);
});
