 ...
 * v0.6g: Extended bleed — board surface extends 2x beyond playable area with fade.
 */
...
(function addSvgBoardSurface() {
  ...
  // ── Extended bleed zone ──
  // Board surface extends far beyond 720×528 so you never see the edge.
  // The outer bleed fades to transparent via a radial gradient mask,
  // blending seamlessly into the fog-base colour beneath.
  var BLEED = 400; // SVG units of extra ground in each direction
  var bx = -BLEED, by = -BLEED;
  var bw = 720 + BLEED * 2, bh = 528 + BLEED * 2;

  // Radial gradient mask: solid in the playable area, fades at bleed edges
  ...
  // Extended board surface with bleed + fade mask
  ...
  // Grid overlay — only on the playable area (720×528)
