...
(function addSvgBoardSurface() {
  ...
  // Extended world surface — includes staging / deep strike / reserves,
  // then fades to black well beyond all gameplay areas.
  var WORLD_X = -760;
  var WORLD_Y = -180;
  var WORLD_W = 1700;
  var WORLD_H = 900;

  var worldBg = document.createElementNS(NS, 'rect');
  worldBg.setAttribute('x', String(WORLD_X));
  worldBg.setAttribute('y', String(WORLD_Y));
  worldBg.setAttribute('width', String(WORLD_W));
  worldBg.setAttribute('height', String(WORLD_H));
  worldBg.setAttribute('fill', '#070b11');
  terrainSvg.insertBefore(worldBg, terrainSvg.firstChild);

  // Slightly brighter core board for readability
  ...
  worldBg.after(boardBg);

  // Grid overlay only on playable board
  ...
  // Fade the whole world surface to black at its true outer bounds.
  var WORLD_FADE_X = 420;
  var WORLD_FADE_Y = 220;
  var vigColor = '#000000';
  var gradDefs = [
  ...
  gradDefs.forEach(function(g) {
    ...
    lg.innerHTML = '<stop offset="0%" stop-color="' + vigColor + '" stop-opacity="1"/>' +
      '<stop offset="45%" stop-color="' + vigColor + '" stop-opacity="0.55"/>' +
  ...
  [
    { id: 'vig-l', x: WORLD_X, y: WORLD_Y, w: WORLD_FADE_X, h: WORLD_H },
    { id: 'vig-r', x: WORLD_X + WORLD_W - WORLD_FADE_X, y: WORLD_Y, w: WORLD_FADE_X, h: WORLD_H },
    { id: 'vig-t', x: WORLD_X, y: WORLD_Y, w: WORLD_W, h: WORLD_FADE_Y },
    { id: 'vig-b', x: WORLD_X, y: WORLD_Y + WORLD_H - WORLD_FADE_Y, w: WORLD_W, h: WORLD_FADE_Y }
  ].forEach(function(s) {
