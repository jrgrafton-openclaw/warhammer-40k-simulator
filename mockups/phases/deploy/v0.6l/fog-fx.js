...
(function initFogForeground() {
  ...
  function initWisps(w, h) {
    ...
    for (var i = 0; i < NUM_WISPS; i++) {
      wisps.push({
        ...
        radiusX: 120 + Math.random() * 220,
        radiusY: 16 + Math.random() * 32,
  ...
  function initExplosions(w, h) {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      explosions.push({
        ...
        timer: 320 + Math.random() * 560, flashAlpha: 0, glowAlpha: 0,
        radius: 10 + Math.random() * 8, maxFlash: 0.16 + Math.random() * 0.12, colors: c
      });
    }
  }
    ...
    for (var i = 0; i < explosions.length; i++) {
      ...
      switch (ex.phase) {
        case 'dormant':
          ...
          if (ex.timer <= 0) { ex.phase = 'flash'; ex.timer = 5 + Math.random() * 6; }
          break;
        case 'flash':
        ...
        case 'afterglow':
          ...
          if (ex.timer <= 0) { ex.phase = 'dormant'; ex.timer = 520 + Math.random() * 700; }
          break;
      }
    }
  }
  ...
  function checkExplosions() {
    for (var i = 0; i < explosions.length; i++) {
      ...
      var isFlashing = ex.phase === 'flash' && ex.flashAlpha > 0.18;
  ...
  function draw() {
    ...
    for (var d = 0; d < debris.length; d++) {
      ...
      var dx = db.x + pan.tx * 0.04;
      var dy = db.y + pan.ty * 0.04;
      ...
      if (db.isEmber) {
        ...
        ctx.fill();
        // Tiny glow
        ctx.fillStyle = 'rgba(255,120,20,' + (ea * 0.18) + ')';
        ctx.beginPath();
