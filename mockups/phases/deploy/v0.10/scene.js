/**
 * scene.js — Army data + initialisation wiring for deployment v0.10.
...
// ── Apply scatter terrain textures ───────────────────────
// Uses AI-generated scatter terrain texture (crates/sandbags top-down).
// Each scatter piece gets a different patternTransform offset to look unique.
(function applyScatterTextures() {
  var terrainSvg = document.getElementById('bf-svg-terrain');
  if (!terrainSvg) return;
  var NS = 'http://www.w3.org/2000/svg';
  ...
  // Create multiple pattern instances with different offsets
  var scatterCount = 0;
  ...
  allPaths.forEach(function(p) {
    ...
    // Scatter terrain paths: rgba(58,48,24,0.7)
    if (fill.indexOf('58,48,24') === -1) return;

    var patId = 'scatter-tex-' + scatterCount;
    scatterCount++;

    // Create a unique pattern with random offset for this instance
    var pat = document.createElementNS(NS, 'pattern');
    pat.setAttribute('id', patId);
    pat.setAttribute('patternUnits', 'userSpaceOnUse');
    pat.setAttribute('width', '96');
    pat.setAttribute('height', '96');
    // Random offset + rotation to make each instance look different
    var offsetX = (scatterCount * 37) % 96;
    var offsetY = (scatterCount * 53) % 96;
    var rotation = (scatterCount * 90) % 360;
    pat.setAttribute('patternTransform', 'translate(' + offsetX + ',' + offsetY + ') rotate(' + rotation + ')');

    // Texture image (darkened via opacity)
    var img = document.createElementNS(NS, 'image');
    img.setAttribute('href', 'scatter-terrain.jpg');
    img.setAttribute('width', '96');
    img.setAttribute('height', '96');
    img.setAttribute('opacity', '0.55');
    pat.appendChild(img);

    // Dark overlay rect to dim the texture further
    var overlay = document.createElementNS(NS, 'rect');
    overlay.setAttribute('width', '96');
    overlay.setAttribute('height', '96');
    overlay.setAttribute('fill', 'rgba(30,25,15,0.45)');
    pat.appendChild(overlay);

    defs.appendChild(pat);

    // Apply the texture pattern to this scatter path
