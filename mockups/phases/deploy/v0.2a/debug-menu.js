...
(function initDebugMenu() {
  ...
  var defaults = {
    ...
    wispsOn: true,
    offboardBorders: false,
    zoneVigDepth: 80, zoneVigOpacity: 0.25, zoneVigActiveOpacity: 0.55
  };
  ...
  sliderRow(zoneBody, 'Vignette Opacity (inactive)', 0, 1, 0.01, state.zoneVigOpacity, '', function(v) {
    state.zoneVigOpacity = v; applyZoneVignettes(); save(state);
  });
  sliderRow(zoneBody, 'Vignette Opacity (active)', 0, 1, 0.01, state.zoneVigActiveOpacity, '', function(v) {
    state.zoneVigActiveOpacity = v; applyZoneVignettes(); save(state);
  });
  ...
  // ══════════════════════════════════════════════════════
  // Spark sub-section
  subLabel(fxBody, 'SPARKS');
  toggleRow(fxBody, 'Enabled', state.sparksOn, function(on) {
    state.sparksOn = on; applyFx(); save(state);
  });
  ...
  function applyOffboard() {
    var zoneInfo = [
      { cls: 'staging-zone-bg', color: '0,212,255' },
      { cls: 'ds-zone-bg', color: '255,170,0' },
      { cls: 'reserves-zone-bg', color: '186,126,255' }
    ];
  ...
  function applyZoneVignettes() {
    ...
    var opacity = state.zoneVigOpacity != null ? state.zoneVigOpacity : 0.5;
    var activeOpacity = state.zoneVigActiveOpacity != null ? state.zoneVigActiveOpacity : 0.8;

    // Store active opacity for deployment.js to read
    window.__zoneVigActiveOpacity = activeOpacity;
    ...
    zoneVigKeys.forEach(function(key) {
      ...
      suffixes.forEach(function(s) {
        ...
        if (!r) return;
        // Update gradient first stop opacity
        if (g) {
          var stops = g.querySelectorAll('stop');
          if (stops.length > 0) stops[0].setAttribute('stop-opacity', String(opacity));
          if (stops.length > 1) stops[1].setAttribute('stop-opacity', String(opacity * 0.3));
        }
        // Update rect dimensions based on depth
        if (s === '-l') {
    ...
    // Inject CSS for active state — when .zone-active is on, boost vignette opacity
    var zvStyle = document.getElementById('zone-vig-active-style');
    if (!zvStyle) {
      zvStyle = document.createElement('style');
      zvStyle.id = 'zone-vig-active-style';
      document.head.appendChild(zvStyle);
    }
    // We'll handle active via JS in deployment.js reading window.__zoneVigActiveOpacity
  }
