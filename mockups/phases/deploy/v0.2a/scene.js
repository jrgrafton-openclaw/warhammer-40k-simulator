...
(function addSvgBoardSurface() {
  ...
  function zoneGrad(id, color) {
    return '<radialGradient id="' + id + '" cx="50%" cy="50%" r="55%">' +
      '<stop offset="0%" stop-color="rgb(' + color + ')" stop-opacity="0.04"/>' +
      '<stop offset="100%" stop-color="rgb(' + color + ')" stop-opacity="0"/>' +
      '</radialGradient>';
  }
  defs.innerHTML += zoneGrad('zone-staging-grad', '0,212,255') +
    zoneGrad('zone-staging-grad-active', '0,212,255') +
    zoneGrad('zone-ds-grad', '255,170,0') +
    zoneGrad('zone-ds-grad-active', '255,170,0') +
    zoneGrad('zone-reserves-grad', '186,126,255') +
    zoneGrad('zone-reserves-grad-active', '186,126,255');
  ...
  // ── Zone vignettes — black edge fades per off-board zone (same style as board) ──
  var zoneVigInfo = [
  ...
  zoneVigInfo.forEach(function(zi) {
    [
      ...
      lg.innerHTML =
        '<stop offset="0%" stop-color="' + vigColor + '" stop-opacity="0.95"/>' +
        '<stop offset="35%" stop-color="' + vigColor + '" stop-opacity="0.4"/>' +
        '<stop offset="100%" stop-color="' + vigColor + '" stop-opacity="0"/>';
      vigDefs.appendChild(lg);
    });
