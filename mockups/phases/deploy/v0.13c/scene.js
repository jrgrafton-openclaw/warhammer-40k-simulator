/**
 * scene.js — Army data + initialisation wiring for deployment v0.13c.
 * ES module entry point. Full battlefield single image approach.
 * One AI-generated image covers the entire board as background.
 */
...
(function addSvgBoardSurface() {
  ...
  // ── Full battlefield image as board surface ──
  // Single AI-generated image covers the entire 720x528 board.
  ...
  boardBase.setAttribute('fill', 'rgba(6,10,16,1)');
  terrainSvg.insertBefore(boardBase, terrainSvg.firstChild);

  // Battlefield image overlay
  var bfImg = document.createElementNS(NS, 'image');
  bfImg.setAttribute('href', 'battlefield.jpg');
  bfImg.setAttribute('x', '0');
  bfImg.setAttribute('y', '0');
  bfImg.setAttribute('width', '720');
  bfImg.setAttribute('height', '528');
  bfImg.setAttribute('preserveAspectRatio', 'xMidYMid slice');
  bfImg.setAttribute('opacity', '1');
  boardBase.after(bfImg);
  ...
  bfImg.after(gridRect);
})();

// ── Initialise shared modules ────────────────────────────
renderTerrain();

// ── v0.13c: Make terrain paths nearly invisible ──────────
// The battlefield image already renders the terrain visually.
// Keep paths for hitbox/collision but make them very faint.
(function fadeTerrainPaths() {
  ...
  allPaths.forEach(function(p) {
    ...
    if (fill.indexOf('106,114,114') !== -1) {
      // Wall paths — very faint outline
      p.setAttribute('fill', 'none');
      p.setAttribute('stroke', 'rgba(100,110,120,0.15)');
      p.setAttribute('stroke-width', '0.5');
    } else if (fill.indexOf('58,64,64') !== -1) {
      // Floor paths — invisible (image has the floor)
      p.setAttribute('fill', 'none');
...
// ── v0.13c: No shadows — battlefield image handles all visuals ──
