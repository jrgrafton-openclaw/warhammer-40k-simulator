 ...
 * v0.6h: Edge vignette — board edges dissolve into fog via gradient overlays.
 */
...
(function addSvgBoardSurface() {
  ...
  // ── Edge vignette — darken board edges so they dissolve into fog ──
  // Four gradient rects, one per edge, layered on top of everything in terrain SVG.
  var VIGNETTE_DEPTH = 120; // SVG units of fade inward from each edge
  var vigColor = '#2a2830'; // matches fog-base

  // Left edge
  var vigL = document.createElementNS(NS, 'linearGradient');
  vigL.setAttribute('id', 'vig-l'); vigL.setAttribute('x1', '0'); vigL.setAttribute('y1', '0');
  vigL.setAttribute('x2', '1'); vigL.setAttribute('y2', '0');
  vigL.innerHTML = '<stop offset="0%" stop-color="' + vigColor + '" stop-opacity="0.9"/><stop offset="100%" stop-color="' + vigColor + '" stop-opacity="0"/>';
  defs.appendChild(vigL);

  // Right edge
  var vigR = document.createElementNS(NS, 'linearGradient');
  vigR.setAttribute('id', 'vig-r'); vigR.setAttribute('x1', '1'); vigR.setAttribute('y1', '0');
  vigR.setAttribute('x2', '0'); vigR.setAttribute('y2', '0');
  vigR.innerHTML = '<stop offset="0%" stop-color="' + vigColor + '" stop-opacity="0.9"/><stop offset="100%" stop-color="' + vigColor + '" stop-opacity="0"/>';
  defs.appendChild(vigR);

  // Top edge
  var vigT = document.createElementNS(NS, 'linearGradient');
  vigT.setAttribute('id', 'vig-t'); vigT.setAttribute('x1', '0'); vigT.setAttribute('y1', '0');
  vigT.setAttribute('x2', '0'); vigT.setAttribute('y2', '1');
  vigT.innerHTML = '<stop offset="0%" stop-color="' + vigColor + '" stop-opacity="0.9"/><stop offset="100%" stop-color="' + vigColor + '" stop-opacity="0"/>';
  defs.appendChild(vigT);

  // Bottom edge
  var vigB = document.createElementNS(NS, 'linearGradient');
  vigB.setAttribute('id', 'vig-b'); vigB.setAttribute('x1', '0'); vigB.setAttribute('y1', '1');
  vigB.setAttribute('x2', '0'); vigB.setAttribute('y2', '0');
  vigB.innerHTML = '<stop offset="0%" stop-color="' + vigColor + '" stop-opacity="0.9"/><stop offset="100%" stop-color="' + vigColor + '" stop-opacity="0"/>';
  defs.appendChild(vigB);

  // Create vignette rects (appended AFTER terrain so they overlay everything)
