...
// ── Pan limits — truly centered, verified by testing ──
// 
// HOW THE PAN SYSTEM WORKS:
// - #battlefield-inner has position:absolute, inset:0 → fills #battlefield
// - It's styled with transform: translate(tx, ty) scale(s)
// - The SVGs inside have viewBox="0 0 720 528" and width/height 100%
// - So the SVG maps 720 units → clientWidth of battlefield, 528 → clientHeight
// - At scale 1.0: 1 SVG unit = (clientWidth / 720) px
// - At scale 0.5: the whole thing renders at half size
//
// WHERE IS THE BOARD ON SCREEN?
// The board's SVG origin (0,0) maps to screen position (tx, ty) after scaling.
// Board center (360, 264) maps to screen position:
//   screenX = tx + 360 * (clientWidth/720) * scale  (but actually it's simpler)
//
// Wait — because battlefield-inner fills the viewport AND uses scale,
// the SVG viewBox coordinates map through the viewport dimensions:
//   pxPerUnit = clientWidth / 720  (before scale)
//   After scale: 1 SVG unit = pxPerUnit * scale display pixels
//   Board center screen pos = tx + 360 * pxPerUnit * scale
//
// But pxPerUnit depends on the viewport size. This is getting complex.
// SIMPLER APPROACH: just measure the actual element bounding rect.
//
(function addPanLimits() {
  ...
  function clampPan() {
    ...
    // The battlefield-inner is bfW x bfH unscaled.
    // After scale, it renders as (bfW * curScale) x (bfH * curScale).
    // The SVG viewBox 720x528 maps to bfW x bfH (fills the container).
    // So the board (720x528 SVG) renders at (bfW * curScale) x (bfH * curScale) px.

    var renderedW = bfW * curScale;
    var renderedH = bfH * curScale;

    // Center position: tx/ty values that would center the scaled content in the viewport.
    // Centered means: the scaled content is in the middle of the viewport.
    var centerTx = (bfW - renderedW) / 2;
    var centerTy = (bfH - renderedH) / 2;

    // Allow panning ± half the rendered dimension from center (so the edge can reach the center)
    var maxPanX = renderedW * 0.5;
    var maxPanY = renderedH * 0.5;

    var clampedTx = Math.max(centerTx - maxPanX, Math.min(centerTx + maxPanX, curTx));
    var clampedTy = Math.max(centerTy - maxPanY, Math.min(centerTy + maxPanY, curTy));
  ...
  var observer = new MutationObserver(function(mutations) {
    for (var i = 0; i < mutations.length; i++) {
      if (mutations[i].attributeName === 'style') { clampPan(); break; }
    }
  });
...
// Center the board vertically in the viewport.
// Horizontally offset to show staging zone (tx=350).
var inner = document.getElementById('battlefield-inner');
if (inner) {
  var bf = document.getElementById('battlefield');
  var bfH = bf ? bf.clientHeight : 700;
  var renderedH = bfH * 0.5; // at scale 0.5
  var centerTy = (bfH - renderedH) / 2;
  inner.style.transform = 'translate(350px, ' + centerTy + 'px) scale(0.5)';
}
