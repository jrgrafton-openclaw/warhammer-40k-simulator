...
// Each terrain path gets a unique <pattern> fill with the texture image.
// Pattern is sized to the path's bounding box so the image covers the terrain.
(function applyTerrainTextures() {
  ...
  allGroups.forEach(function(g) {
    ...
    var idx = isRuin ? ruinIdx : scatterIdx;
    var imgFile = isRuin ? 'ruin-' + (idx % 8) + '.jpg' : 'scatter-' + (idx % 8) + '.jpg';

    // Create a unique pattern for each terrain piece
    paths.forEach(function(p, pIdx) {
      var patId = (isRuin ? 'ruin-pat-' + ruinIdx : 'scatter-pat-' + scatterIdx) + '-' + pIdx;
      var pat = document.createElementNS(NS, 'pattern');
      pat.setAttribute('id', patId);
      pat.setAttribute('patternUnits', 'userSpaceOnUse');
      pat.setAttribute('x', String(minX));
      pat.setAttribute('y', String(minY));
      pat.setAttribute('width', String(bw));
      pat.setAttribute('height', String(bh));
