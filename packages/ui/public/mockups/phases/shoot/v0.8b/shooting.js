(function(){
  ...
  function ensureOverlayPinLoop(){
    ...
    const tick = () => {
      ...
      if (popup && !popup.classList.contains('hidden')) state.overlayRaf = requestAnimationFrame(tick);
      else state.overlayRaf = null;
    };
    state.overlayRaf = requestAnimationFrame(tick);
  }

  function bindUnitCardChrome(){
    $('#card-close')?.addEventListener('click', () => $('#unit-card')?.classList.remove('visible'));
  }

  function takeoverUnitCard(html){
    const card = $('#unit-card');
    if (!card) return null;
    if (savedUnitCardHTML == null) savedUnitCardHTML = card.innerHTML;
    card.innerHTML = html;
    card.classList.add('card-takeover', 'visible');
    return card;
  }

  function restoreUnitCard(){
    const card = $('#unit-card');
    if (!card || savedUnitCardHTML == null) return;
    card.innerHTML = savedUnitCardHTML;
    savedUnitCardHTML = null;
    card.classList.remove('card-takeover');
    bindUnitCardChrome();
  }
  ...
  function renderDiceStage(title, count, threshold, auto, message='', ctaLabel='Click to roll'){
    ...
    takeoverUnitCard(`<div class="overlay-title">${title}</div><div class="dice-row">${chips}</div><div class="dice-summary">${message || (threshold ? `Target ${threshold}+` : 'Resolve damage')}</div><button class="roll-cta">${auto ? 'Resolving…' : ctaLabel}</button>`);
  }

  function revealDice(rolls, threshold, stageKind){
    const chips = $$('#unit-card .die');
    rolls.forEach((r, i) => {
  ...
  function rollDiceStage(title, rolls, threshold, auto = false, targetId = null, message='', stageKind='generic', ctaLabel='Click to roll', nextLabel='Continue', onTrigger = null){
    return new Promise(resolve => {
      const card = $('#unit-card'); if (!card) return resolve({ rolls, successes: rolls.length, threshold });
      ...
      const cta = card.querySelector('.roll-cta');
      const fire = async () => {
  ...
  function showResultPanel(targetId, totalDamage, killCount){
    return new Promise(resolve => {
      ...
      card?.querySelector('.roll-cta')?.addEventListener('click', () => {
        restoreUnitCard();
        resolve();
      }, { once: true });
    });
