/* WH40K Battle UI — Units & Card module v1 */
(function(B){
  ...
  B.initAllTooltips = function() {
    document.querySelectorAll('[data-tip]').forEach(function(el) {
      if (el._tipInit) return;
      el._tipInit = true;
      el.addEventListener('mouseenter', function() { B.showTip(el, el.dataset.tip); });
  ...
  B.buildCard = function(uid) {
    ...
    if (u.wargear && u.wargear.length) {
      ...
      wgEl.querySelectorAll('.wg-row').forEach(function(r) {
        r.addEventListener('click', function() {
          if (!B.wgState[uid]) B.wgState[uid]={};
          B.wgState[uid][+r.dataset.i] = !B.wgState[uid][+r.dataset.i];
          B.buildCard(uid);
        });
      });
    } else { wgEl.innerHTML=''; }
    ...
    if (u.wargear && B.wgState[uid]) {
      u.wargear.forEach(function(wg,i){ if(B.wgState[uid][i]&&wg.adds) weapons.push(wg.adds); });
    }
    var types = [];
    ...
    types.forEach(function(type) {
      ...
      weapons.filter(function(w){return w.type===type;}).forEach(function(w) {
        wHtml += '<div class="wt-row"><div class="wt-name-row"><span class="wt-name">' + w.name + '</span><span class="wt-val">' + w.rng + '</span><span class="wt-val">' + w.a + '</span><span class="wt-val">' + w.s + '</span><span class="wt-val ' + B.apCls(w.ap) + '">' + w.ap + '</span><span class="wt-val dmg">' + w.d + '</span></div><div class="wt-kws">' + (w.kw||[]).map(B.kwPill).join('') + '</div></div>';
      });
    });
    ...
    (u.abilities||[]).forEach(function(a) {
      aHtml += '<div class="ability-row"><div class="ability-pill">' + a.name + '</div><div class="ability-desc">' + B.formatDesc(a.desc) + '</div></div>';
    });
