...
function highlightAllZonesByDetection(activeZoneName) {
  ...
  // Off-board zones always highlight during any drag (they're always valid drop targets)
  var isDragging = activeZoneName !== null && activeZoneName !== undefined;
  if (stagingZone) stagingZone.classList.toggle('zone-active', isDragging);
  if (dsZone) dsZone.classList.toggle('zone-active', isDragging);
  if (reservesZone) reservesZone.classList.toggle('zone-active', isDragging);
}
