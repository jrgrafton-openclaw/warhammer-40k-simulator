...
(function addSvgBoardSurface() {
  ...
  // v0.1c: OVERSIZED GROUND + VIGNETTE
  // Board surface extends 50% beyond the play area on each side.
  // A radial gradient vignette darkens the outer edges to blend
  // seamlessly into the space background (near-black).
  var EXT = 360; // 50% of 720 width
  var EXT_Y = 264; // 50% of 528 height

  // Extended dark board surface
  var boardBg = document.createElementNS(NS, 'rect');
  boardBg.setAttribute('x', String(-EXT));
  boardBg.setAttribute('y', String(-EXT_Y));
  boardBg.setAttribute('width', String(720 + EXT * 2));
  boardBg.setAttribute('height', String(528 + EXT_Y * 2));
  boardBg.setAttribute('fill', 'rgba(8,14,22,0.88)');
  terrainSvg.insertBefore(boardBg, terrainSvg.firstChild);

  // Extended grid overlay — grid fades out naturally via the vignette
  var gridRect = document.createElementNS(NS, 'rect');
  gridRect.setAttribute('x', String(-EXT));
  gridRect.setAttribute('y', String(-EXT_Y));
  gridRect.setAttribute('width', String(720 + EXT * 2));
  gridRect.setAttribute('height', String(528 + EXT_Y * 2));
  gridRect.setAttribute('fill', 'url(#board-grid)');
  gridRect.setAttribute('pointer-events', 'none');
  boardBg.after(gridRect);

  // Create vignette radial gradient in defs
  var vigGrad = document.createElementNS(NS, 'radialGradient');
  vigGrad.setAttribute('id', 'board-vignette');
  vigGrad.setAttribute('cx', '50%');
  vigGrad.setAttribute('cy', '50%');
  vigGrad.setAttribute('r', '50%');
  vigGrad.setAttribute('fx', '50%');
  vigGrad.setAttribute('fy', '50%');

  // Inner area: fully transparent (play area visible)
  var stop1 = document.createElementNS(NS, 'stop');
  stop1.setAttribute('offset', '0.35');
  stop1.setAttribute('stop-color', 'rgba(4,8,16,0)');
  vigGrad.appendChild(stop1);

  // Transition zone: starts darkening
  var stop2 = document.createElementNS(NS, 'stop');
  stop2.setAttribute('offset', '0.55');
  stop2.setAttribute('stop-color', 'rgba(4,8,16,0.3)');
  vigGrad.appendChild(stop2);

  // Outer zone: heavy darkness
  var stop3 = document.createElementNS(NS, 'stop');
  stop3.setAttribute('offset', '0.75');
  stop3.setAttribute('stop-color', 'rgba(4,8,16,0.75)');
  vigGrad.appendChild(stop3);

  // Edge: near-opaque — blends into the space background
  var stop4 = document.createElementNS(NS, 'stop');
  stop4.setAttribute('offset', '1.0');
  stop4.setAttribute('stop-color', 'rgba(4,8,16,0.95)');
  vigGrad.appendChild(stop4);

  defs.appendChild(vigGrad);

  // Vignette overlay rect — same size as extended board
  var vigRect = document.createElementNS(NS, 'rect');
  vigRect.setAttribute('x', String(-EXT));
  vigRect.setAttribute('y', String(-EXT_Y));
  vigRect.setAttribute('width', String(720 + EXT * 2));
  vigRect.setAttribute('height', String(528 + EXT_Y * 2));
  vigRect.setAttribute('fill', 'url(#board-vignette)');
  vigRect.setAttribute('pointer-events', 'none');
  // Insert at the end of terrain SVG (on top of everything in that layer)
  terrainSvg.appendChild(vigRect);
})();
