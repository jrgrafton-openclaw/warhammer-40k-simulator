(function(){
  ...
  const state = {
    ...
    overlayRaf: null,
    ghostOverlayTimer: null,
    pinnedRollPosition: null
  };
  ...
  function computeDodgePosition(attackerId, targetId){
    const field = battlefieldRect();
    const inner = battlefieldInnerRect();
    if (!field || !inner || !attackerId || !targetId) return { x: 0, y: 0, valid: false };

    const attacker = getUnit(attackerId);
    const target = getUnit(targetId);
    const attackerCenter = attacker ? center(attacker) : null;
    const targetCenter = target ? center(target) : null;
    if (!attackerCenter?.valid || !targetCenter?.valid) return { x: 0, y: 0, valid: false };

    const attackerPos = toBattlefieldCoords(attackerCenter.x, attackerCenter.y);
    const targetPos = toBattlefieldCoords(targetCenter.x, targetCenter.y);
    if (!attackerPos.valid || !targetPos.valid) return { x: 0, y: 0, valid: false };

    const innerLeft = inner.left - field.left;
    const innerTop = inner.top - field.top;
    ...
    const attackMidpoint = {
      x: (attackerPos.x + targetPos.x) / 2,
      y: (attackerPos.y + targetPos.y) / 2
    };

    const quadrants = [
      { x: innerLeft + innerWidth * 0.25, y: innerTop + innerHeight * 0.25 },
      { x: innerLeft + innerWidth * 0.75, y: innerTop + innerHeight * 0.25 },
      { x: innerLeft + innerWidth * 0.25, y: innerTop + innerHeight * 0.75 },
      { x: innerLeft + innerWidth * 0.75, y: innerTop + innerHeight * 0.75 }
    ];

    const scored = quadrants.map((pos) => {
      const distToAttacker = Math.hypot(pos.x - attackerPos.x, pos.y - attackerPos.y);
      const distToTarget = Math.hypot(pos.x - targetPos.x, pos.y - targetPos.y);
      const distToMidpoint = Math.hypot(pos.x - attackMidpoint.x, pos.y - attackMidpoint.y);
      return {
        ...pos,
        score: Math.min(distToAttacker, distToTarget),
        distToMidpoint
      };
    }).sort((a, b) => {
      if (b.score !== a.score) return b.score - a.score;
      return b.distToMidpoint - a.distToMidpoint;
    });

    const best = scored[0];
    if (!best) return { x: 0, y: 0, valid: false };

    const minX = innerLeft + halfW;
    const maxX = innerLeft + innerWidth - halfW;
    const minY = innerTop + 14;
    const maxY = innerTop + innerHeight - overlayHeight - 14;

    return {
      x: Math.max(minX, Math.min(best.x, maxX)),
      y: Math.max(minY, Math.min(best.y - halfH, maxY)),
      valid: true
    };
  }

  function ghostOverlay(duration = 500){
    const overlay = $('#roll-overlay');
    if (!overlay) return;
    if (state.ghostOverlayTimer) clearTimeout(state.ghostOverlayTimer);
    overlay.classList.add('ghost');
    state.ghostOverlayTimer = setTimeout(() => {
      overlay.classList.remove('ghost');
      state.ghostOverlayTimer = null;
    }, duration);
  }
  ...
  function ensureOverlayPinLoop(){
    ...
    const tick = () => {
      ...
      if (roll && !roll.classList.contains('hidden') && state.pinnedRollTargetId) {
        const a = state.pinnedRollPosition || computeDodgePosition(state.attackerId, state.pinnedRollTargetId);
        if (a.valid) {
          state.pinnedRollPosition = a;
          roll.style.left = `${a.x}px`;
          roll.style.top = `${a.y}px`;
        }
      }
  ...
  function rollDiceStage(title, rolls, threshold, auto = false, targetId = null, message='', stageKind='generic', ctaLabel='Click to roll', nextLabel='Continue', onTrigger = null){
    return new Promise(resolve => {
      ...
      state.pinnedRollPosition = targetId ? computeDodgePosition(state.attackerId, targetId) : null;
      ...
      const fire = async () => {
        ...
        setTimeout(() => {
          if (auto) {
            setTimeout(() => {
              overlay.classList.add('hidden'); state.pinnedRollTargetId = null; state.pinnedRollPosition = null;
  ...
  function showResultPanel(targetId, totalDamage, killCount){
    return new Promise(resolve => {
      ...
      overlay.querySelector('.roll-cta').addEventListener('click', () => {
        overlay.classList.add('hidden'); state.pinnedRollTargetId = null; state.pinnedRollPosition = null; resolve();
      }, { once: true });
    });
  }
